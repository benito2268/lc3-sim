...
namespace lc3 {
    ...
    char* util_funcs::binToStr(std::bitset<16> addr) {
        //read memory until \0 then return c string
       std::vector<char> str;
       int addri = conv::bin16_to_dec(addr);
       int current;
       do {
           current = conv::bin16_to_dec(Cpu::mem[addri]);
           str.push_back((char)current);
           addri++;
